...
package Server_Game is
	...
	function Nick_Exists(C4_Game: in C4_Game_Type; Nick: ASU.Unbounded_String) return boolean;
	
	function Nick_EP_Exists(C4_Game: in C4_Game_Type; Nick: ASU.Unbounded_String; EP: LLU.End_Point_Type) return boolean;
	

	private
		type C4_Game_Type is record
			Player_Info: Array_Client_Type;
			Dashboard: access VD.Board_Type := new VD.Board_Type;
